  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        setStatus({
          type: 'success',
          message: 'Chúc mừng! Bài viết của bạn đã được đăng thành công.',
        });
        // Clear form
        setFormData({
          title: '',
        ...
        setStatus({
          type: 'error',
          message: data.message || 'Có lỗi xảy ra trong quá trình đăng bài.',
        });
      }
    } catch (err) {
      console.error(err);
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 space-y-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-6 bg-stone-900/40 border border-stone-850 p-8 rounded-3xl shadow-xl">
          <div className="space-y-2">
            <label className="block text-xs font-semibold uppercase text-stone-400 tracking-wider">Tiêu Đề Bài Viết</label>
            <input
              type="text"
              name="title"
              ...
              placeholder="Ví dụ: Cách chọn hạt Arabica pha cold brew tuyệt hảo..."
            />
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <input
                ...
                required
                className="w-full bg-stone-950 border border-stone-800 focus:border-amber-700 text-stone-200 rounded-xl px-4 py-3 text-sm focus:outline-none transition-colors"
                placeholder="cach-chon-hat-arabica-pha-cold-brew"
              />
            </div>
            <div className="space-y-2">
              <label className="block text-xs font-semibold uppercase text-stone-400 tracking-wider">Tác giả</label>
              <input
                type="text"
                name="author"
          ...
          <div className="space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
              <label className="block text-xs font-semibold uppercase text-stone-400 tracking-wider">Ảnh Bìa Bài Viết</label>
            ...
            <input
              ...
              required
              className="w-full bg-stone-950 border border-stone-800 focus:border-amber-700 text-stone-200 rounded-xl px-4 py-3 text-sm focus:outline-none transition-colors"
              placeholder="Đường dẫn ảnh bìa (https://...) hoặc tự động điền khi tải ảnh lên"
            />
          </div>

          <div className="space-y-2">
            <label className="block text-xs font-semibold uppercase text-stone-400 tracking-wider">Tóm tắt ngắn (Excerpt)</label>
            <input
              type="text"
              name="excerpt"
              ...
              required
              className="w-full bg-stone-950 border border-stone-800 focus:border-amber-700 text-stone-200 rounded-xl px-4 py-3 text-sm focus:outline-none transition-colors"
              placeholder="Tóm tắt nội dung bài viết hiển thị ở trang danh sách tin..."
            />
          </div>

          <div className="space-y-2">
            <label className="block text-xs font-semibold uppercase text-stone-400 tracking-wider">Nội dung chi tiết (HTML hoặc Text)</label>
            <textarea
              name="content"
              ...
              required
              rows="10"
              className="w-full bg-stone-950 border border-stone-800 focus:border-amber-700 text-stone-200 rounded-xl px-4 py-3 text-sm focus:outline-none transition-colors resize-none font-sans"
              placeholder="<p>Nhập nội dung bài viết của bạn tại đây, hỗ trợ mã HTML...</p>"
            ></textarea>
          </div>
